...
#
# v2.3 Task 10 — Phase 1 PL-detection routing in questioning-flow.md.
# Verifies the reference adds a Phase 1 sub-section that routes a
# "designing a programming language" intent into one of the 6 PL sub_types
# and cross-references the follow-up Phase 2 batches (impl_strategy + host_runtime)
# and Phase 4 template generation.
...
# v2.3 marker so future readers know what added this routing.
assert_contains "$CONTENT" 'v2.3' 'questioning-flow.md must carry a v2.3 marker for the PL-routing section'
...
# Routing rule: must explicitly point at state.decisions.project.sub_type
# (or state.decisions ... project.sub_type) so the reader knows where the
# answer is persisted.
assert_contains "$CONTENT" 'state.decisions' 'questioning-flow.md must persist the PL sub_type into state.decisions'
assert_contains "$CONTENT" 'sub_type' 'questioning-flow.md must mention sub_type assignment for PL routing'

# Cross-reference to Phase 2 follow-up batches (impl_strategy + host_runtime
# are the next two PL axes introduced in v2.3).
assert_contains "$CONTENT" 'Phase 2' 'questioning-flow.md must cross-reference Phase 2 for PL follow-up batches'
...
# Cross-reference to Phase 4 where the 7 PL templates generate (v2.3 sketch F).
assert_contains "$CONTENT" 'Phase 4' 'questioning-flow.md must cross-reference Phase 4 (where the 7 PL templates generate)'
