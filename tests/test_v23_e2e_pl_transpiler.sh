...
#
# E2E fixture test (programming-language transpiler-to-JS DSL, Rust-host
# transpiler emitting ES2026 JavaScript for a functional DSL called "fern").
# Exercises the full Phase 4 → 5 → 6 → 7 → 8 audit chain by running
# run_all.sh against a representative pre-Phase-4 bundle that selects the PL
# track (Sketch E + Sketch F: transpiler_target sub_type with
# impl_strategy=transpiler, host_runtime=js_host, paradigm=functional,
# type_system=static_gradual).
# (v2.3 Task 14.)

source "$(dirname "$0")/lib/test_helpers.sh"

FIXTURE="$REPO_ROOT/tests/fixtures/e2e-programming-language-transpiler"
RUNNER="$REPO_ROOT/agents/quality-gate-auditor/run_all.sh"

assert_dir_exists "$FIXTURE" "e2e-programming-language-transpiler fixture must exist"
...
# Fixture must encode the PL transpiler track (Sketch E + Sketch F decision keys)
...
assert_eq "$SUB_TYPE" "transpiler_target" "fixture must declare project.sub_type=transpiler_target"
assert_eq "$IMPL" "transpiler" "fixture must declare impl_strategy=transpiler"
assert_eq "$HR" "js_host" "fixture must declare host_runtime=js_host"
assert_eq "$PD" "functional" "fixture must declare paradigm=functional"
assert_eq "$TS" "static_gradual" "fixture must declare type_system=static_gradual"
